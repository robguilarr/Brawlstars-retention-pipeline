...
def players_info_request(
    ...
    """
    Extracts Players metadata from Brawlstars API by executing an Async Event Loop
    over a list of futures objects. These are made of task objects built of Async
    threads due blocking call limitations of api_request sub_module.
    Args:
    ...
    async def spawn_request(player_tags: list) -> pd.DataFrame:
        """Use gathering to request player metadata as async tasks objects, made of
        coroutines"""
        start = time.time()
        ...
        log.info(
            f"Player info request process Finished in {time.time() - start} seconds"
        )
...
def metadata_preparation(
    ...
    """
    Prepare raw players info metadata into a format adaptable (schema) to be
    processed by Spark. Also exclude non-neccesary data for the present analysis.
    Args:
